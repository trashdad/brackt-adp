...
const getStatusColor = (status) => {
  switch (status) {
    case "success":
      return "bg-[#4CAF50] border-black";
    case "error":
      return "bg-[#F44336] border-black";
    case "timeout":
      return "bg-[#FFEB3B] border-black text-black";
    case "running":
      return "bg-[#2196F3] border-black animate-pulse";
    default:
      return "bg-[#555555] border-black";
  }
};
...
return (
  <div className="bg-black flex items-stretch h-12 overflow-hidden border-b-2 border-white/20">
    ...
    <button
      ...
      style={{
        boxShadow:
          "inset -4px -4px 0 0 rgba(0,0,0,0.3), inset 4px 4px 0 0 rgba(255,255,255,0.2)",
      }}
    >
    ...
    <div className="bg-black w-14 border-l-2 border-white/20 flex items-center justify-center">
      <div
        className={`w-3 h-3 rounded-none border border-black ${isRunning ? "bg-red-500 animate-pulse" : "bg-green-900"}`}
      />
    </div>
  </div>
);
