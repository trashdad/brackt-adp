...
export default function SportFilter({ selected = [], onChange }) {
  ...
  return (
    <div className="flex flex-wrap gap-3">
      <button
        onClick={() => onChange([])}
        className={`px-5 py-2.5 border-2 transition-all font-retro text-[11px] tracking-widest active:translate-y-0.5 ${
          selected.length === 0
      ...
      {activeSports.map((sport) => {
        ...
        return (
          <button
            ...
            className={`px-5 py-2.5 border-2 transition-all font-retro text-[11px] tracking-tight active:translate-y-0.5 ${
              isActive
          ...
          >
            <span className="flex items-center gap-2">
              <span className="opacity-80 scale-110">{sport.icon}</span>
